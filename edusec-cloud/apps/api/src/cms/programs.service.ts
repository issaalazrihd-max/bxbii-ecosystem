 ...
 * public /programs page. Platform-wide, not branch-scoped — same as
 * Pages/NavigationItem, since the programs catalog is one shared list for
 * the whole tenant.
 */
@Injectable()
export class ProgramsService {
  ...
  async create(user: AccessTokenPayload, dto: CreateProgramDto) {
    const maxPosition = await this.prisma.program.aggregate({
      where: { tenantId: user.tenantId },
      _max: { position: true },
    });
    const program = await this.prisma.program.create({
      data: {
        ...
        slug: dto.slug,
        arDomain: dto.arDomain,
        enDomain: dto.enDomain,
        arName: dto.arName,
        enName: dto.enName,
  ...
  async reorder(user: AccessTokenPayload, dto: ReorderProgramsDto) {
    ...
    if (dto.orderedProgramIds.length !== known.size || !dto.orderedProgramIds.every((id) => known.has(id))) {
      throw new BadRequestException("orderedProgramIds must contain exactly this tenant's current programs");
    }
    await this.prisma.$transaction(
      dto.orderedProgramIds.map((id, index) =>
        this.prisma.program.update({ where: { id }, data: { position: index } }),
      ),
    );
